...
"%EXECUTABLE%" //IS//%SERVICE_ID% --Startup %MQ_START_TYPE% --StopTimeout %MQ_STOP_TIMEOUT% --StartClass com.alibaba.rocketmq.namesrv.NamesrvStartup --StopClass com.alibaba.rocketmq.namesrv.NamesrvStartup --StartMethod main --StopMethod close --Classpath "%MQ_CLASSPATH%" --JvmSs %JVM_SS% --JvmMs %JVM_XMS% --JvmMx %JVM_XMX% --JvmOptions %JVM_OPTS% ++JvmOptions %MQ_PARAMS% %LOG_OPTS%  --DisplayName "Rocketmq Namesrv %MQ_VERSION% (%SERVICE_ID%)" --Description "Rocketmq Namesrv %MQ_VERSION% Windows Service" --Jvm "%JVM_DLL%" --StartMode jvm --StopMode jvm --StartPath "%MQ_HOME%"
